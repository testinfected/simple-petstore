...
public class SimplePhotoFileStoreTest {
    ...
    photoIsStoredRelativelyToStorageRoot() {
        ...
        assertThat("attachment url", attachmentStorage.getAttachmentUri(withPhoto), equalTo(urlOf(withPhoto.getPhotoFileName())));
    }

    @Test public void
    defaultPhotoIsUsedWhenProductHasNone() {
        Product noPhoto = aProduct().withoutAPhoto().build();
        assertThat("attachment url", attachmentStorage.getAttachmentUri(noPhoto), equalTo(urlOf(DEFAULT_PHOTO)));
    }
